...
static const int FLAGS_ZEROPAD = (1U << 0U);
static const int FLAGS_LEFT = (1U << 1U);
static const int FLAGS_PLUS = (1U << 2U);
static const int FLAGS_SPACE = (1U << 3U);
static const int FLAGS_HASH = (1U << 4U);
static const int FLAGS_UPPERCASE = (1U << 5U);
static const int FLAGS_CHAR = (1U << 6U);
static const int FLAGS_SHORT = (1U << 7U);
static const int FLAGS_INT = (1U << 8U);
// Only used with PRINTF_SUPPORT_MSVC_STYLE_INTEGER_SPECIFIERS
static const int FLAGS_LONG = (1U << 9U);
static const int FLAGS_LONG_LONG = (1U << 10U);
static const int FLAGS_PRECISION = (1U << 11U);
static const int FLAGS_ADAPT_EXP = (1U << 12U);
static const int FLAGS_POINTER = (1U << 13U);
// Note: Similar, but not identical, effect as FLAGS_HASH
static const int FLAGS_SIGNED = (1U << 14U);

static const int FLAGS_INT8 = FLAGS_CHAR;
static const int FLAGS_INT16 = FLAGS_SHORT;
static const int FLAGS_INT32 = FLAGS_INT;
static const int FLAGS_INT64 = FLAGS_LONG_LONG;

static const int BASE_BINARY = 2;
static const int BASE_OCTAL = 8;
static const int BASE_DECIMAL = 10;
static const int BASE_HEX = 16;

static const int PRINTF_SUPPORT_DECIMAL_SPECIFIERS = 1;
static const int PRINTF_SUPPORT_EXPONENTIAL_SPECIFIERS = 1;
static const int PRINTF_SUPPORT_WRITEBACK_SPECIFIER = 1;
static const int PRINTF_SUPPORT_MSVC_STYLE_INTEGER_SPECIFIERS = 1;
static const int PRINTF_SUPPORT_LONG_LONG = 1;
static const int PRINTF_ALIAS_STANDARD_FUNCTION_NAMES_SOFT = 0;
static const int PRINTF_ALIAS_STANDARD_FUNCTION_NAMES_HARD = 0;

static const int PRINTF_INTEGER_BUFFER_SIZE = 32;
static const int PRINTF_DECIMAL_BUFFER_SIZE = 32;
static const int PRINTF_DEFAULT_FLOAT_PRECISION = 6;
static const int PRINTF_MAX_INTEGRAL_DIGITS_FOR_DECIMAL = 9;
static const int PRINTF_LOG10_TAYLOR_TERMS = 4;
static const int PRINTF_CHECK_FOR_NUL_IN_FORMAT_SPECIFIER = 1;

static const bool PRINTF_PREFER_DECIMAL = false;
static const bool PRINTF_PREFER_EXPONENTIAL = true;

static const int PRINTF_MAX_POSSIBLE_BUFFER_SIZE = INT_MAX;

static const float FLT_MAX = __FLT_MAX__;
static const double DBL_MAX = __DBL_MAX__;
static const long double LDBL_MAX = __LDBL_MAX__;

static const double PRINTF_FLOAT_NOTATION_THRESHOLD = 1e9;

static const unsigned int NUM_DECIMAL_DIGITS_IN_INT64_T = 18;
static const unsigned int PRINTF_MAX_PRECOMPUTED_POWER_OF_10 = NUM_DECIMAL_DIGITS_IN_INT64_T;
static const unsigned int PRINTF_MAX_SUPPORTED_PRECISION = NUM_DECIMAL_DIGITS_IN_INT64_T - 1;
...
static const int DOUBLE_SIZE_IN_BITS = 64;

static const int FLT_MANT_DIG = __FLT_MANT_DIG__;
static const int DBL_MANT_DIG = __DBL_MANT_DIG__;
static const int LDBL_MANT_DIG = __LDBL_MANT_DIG__;
static const int DOUBLE_STORED_MANTISSA_BITS = DBL_MANT_DIG - 1;

static const int DOUBLE_EXPONENT_MASK = 0x7FFU;
static const int DOUBLE_BASE_EXPONENT = 1023;

static const int DOUBLE_MAX_SUBNORMAL_EXPONENT_OF_10 = -308;
static const double DOUBLE_MAX_SUBNORMAL_POWER_OF_10 = 1e-308;

static const float FLT_MAX_10_EXP = __FLT_MAX_10_EXP__;
static const double DBL_MAX_10_EXP = __DBL_MAX_10_EXP__;
static const long double LDBL_MAX_10_EXP = __LDBL_MAX_10_EXP__;
...
template<typename T>
T ABS_FOR_PRINTING(T _x) {
    return ((printf_unsigned_value_t) ((_x) > 0 ? (_x) : -((printf_signed_value_t) _x)));
}
...
static inline printf_flags_t parse_flags(const char **format) {
...
static inline printf_size_t atou_(const char **str) {
    printf_size_t i = 0U;
    while (is_digit_(**str)) {
...
static inline void
print_integer_finalization(output_gadget_t *output, char *buf, printf_size_t len, bool negative, numeric_base_t base,
...
static inline void
print_integer(output_gadget_t *output, printf_unsigned_value_t value, bool negative, numeric_base_t base,
              printf_size_t precision, printf_size_t width, printf_flags_t flags) {
...
static inline double log10_of_positive(double positive_number) {
...
static inline int bastardized_floor(double x) {
    if (x >= 0) { return (int) x; }
    int n = (int) x;
    return (((double) n) == x) ? n : n - 1;
}

static inline double pow10_of_int(int floored_exp10) {
...
static inline struct double_components get_components(double number, printf_size_t precision) {
...
static inline double apply_scaling(double num, struct scaling_factor normalization) {
    return normalization.multiply ? num * normalization.raw_factor : num / normalization.raw_factor;
}

static inline double unapply_scaling(double normalized, struct scaling_factor normalization) {
#ifdef __GNUC__
...
static inline struct scaling_factor update_normalization(struct scaling_factor sf, double extra_multiplicative_factor) {
...
static inline struct double_components
get_normalized_components(bool negative, printf_size_t precision, double non_normalized,
...
static inline void
print_exponential_number(output_gadget_t *output, double number, printf_size_t precision, printf_size_t width,
...
static inline void
print_decimal_number(output_gadget_t *output, double number, printf_size_t precision, printf_size_t width,
                     printf_flags_t flags, char *buf, printf_size_t len) {
...
static inline void
print_floating_point(output_gadget_t *output, double value, printf_size_t precision, printf_size_t width,
...
class va_list_gadget_t {
public:
    virtual ~va_list_gadget_t() {}

    virtual void reset_index() = 0;

    virtual int get_int(int &err) = 0;

    virtual long get_long(int &err) = 0;

    virtual long long get_long_long(int &err) = 0;
...
class va_list_gadget_impl_t : public va_list_gadget_t {
public:
    va_list_gadget_impl_t() : index_(0) {
    }
    ...
    void add(int i) {
        args_.push_back(var(i));
    }

    void add(long l) {
        args_.push_back(var(l));
    }

    void add(long long ll) {
        args_.push_back(var(ll));
    }

    void add(double d) {
        args_.push_back(var(d));
    }

    void add(const char *s) {
        args_.push_back(var(s));
    }

    void add(void *p) {
        args_.push_back(var(p));
    }

    virtual int get_int(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
        return args_[index_++].i_;
    }

    virtual long get_long(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
        return args_[index_++].l_;
    }

    virtual long long get_long_long(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
    ...
    virtual double get_double(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
    ...
    virtual const char *get_string(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
    ...
    virtual void *get_pointer(int &err) {
        if (index_ >= args_.size()) {
            err = 1;
            return 0;
        }
...
static inline int format_string_loop(output_gadget_t *output, const char *format, va_list_gadget_t *args) {
    ...
    while (*format) {
        ...
        if (*format == '.') {
            ...
            if (is_digit_(*format)) {
                ...
                const int precision_ = args->get_int(err);
                CHECK_ERR_IN_FORMAT_STRING(err);
        ...
        switch (*format) {
            ...
            case 'b' : {
                ...
                if (flags & FLAGS_SIGNED) {
                    ...
                    if (flags & FLAGS_LONG_LONG) {
                        ...
                        const int value =
                                (flags & FLAGS_CHAR) ? (signed char) args->get_int(err) :
                                (flags & FLAGS_SHORT) ? (short int) args->get_int(err) :
                                args->get_int(err);
                        print_integer(output, ABS_FOR_PRINTING(value), value < 0, base, precision, width, flags);
                    }
                } else {
                    ...
                    if (flags & FLAGS_LONG_LONG) {
                        print_integer(output, (printf_unsigned_value_t) (unsigned long long) args->get_long_long(err),
                                      false, base,
                                      precision, width, flags);
                    } else if (flags & FLAGS_LONG) {
                        print_integer(output, (printf_unsigned_value_t) (unsigned long) args->get_long(err), false,
                                      base,
                                      precision, width, flags);
                    } else {
                        const unsigned int value =
                                (flags & FLAGS_CHAR) ? (unsigned char) (unsigned int) args->get_int(err) :
                                (flags & FLAGS_SHORT) ? (unsigned short int) (unsigned int) args->get_int(err) :
                                (unsigned int) args->get_int(err);
            ...
            case 'F' :
                ...
                print_floating_point(output, args->get_double(err), precision, width, flags, PRINTF_PREFER_DECIMAL);
                format++;
                break;
            case 'e':
            case 'E':
            ...
            case 'G':
                ...
                print_floating_point(output, args->get_double(err), precision, width, flags, PRINTF_PREFER_EXPONENTIAL);
                format++;
                break;
            case 'c' : {
                printf_size_t l = 1U;
                ...
                // char output
                putchar_via_gadget(output, (char) args->get_int(err));
                // post padding
                if (flags & FLAGS_LEFT) {
                    while (l++ < width) {
                        putchar_via_gadget(output, ' ');
            ...
            case 'p' : {
                ...
                uintptr_t value = (uintptr_t) args->get_pointer(err);
            ...
            case 'n' : {
                if (flags & FLAGS_CHAR) *((char *) args->get_string(err)) = (char) output->pos;
                else if (flags & FLAGS_SHORT) *((short *) args->get_pointer(err)) = (short) output->pos;
                else if (flags & FLAGS_LONG) *((long *) args->get_pointer(err)) = (long) output->pos;
                else if (flags & FLAGS_LONG_LONG) *((long long *) args->get_pointer(err)) = (long long int) output->pos;
                else *((int *) args->get_pointer(err)) = (int) output->pos;
                format++;
                break;
            }
...
static inline int vsnprintf_impl(output_gadget_t *output, const char *format, va_list_gadget_t *args) {
...
static inline int printfv(const char *fmt, va_list_gadget_t *args) {
    output_gadget_t gadget = function_gadget(putchar_wrapper, NULL);
...
static inline int snprintfv(char *s, size_t n, const char *format, va_list_gadget_t *args) {
    output_gadget_t gadget = buffer_gadget(s, n);
    return vsnprintf_impl(&gadget, format, args);
}
